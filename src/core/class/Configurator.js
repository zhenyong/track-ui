...
// @require Tk.Config
    ...
    /**
     * @property {Tk.Class} cls The class to which this instance is associated.
     * @private
     * @readonly
     */
    me.cls = cls;
     ...
     *
     * @property {Tk.Configurator} superCfg
     * @private
     * @readonly
     */
    ...
    if (superCfg) {
        /**
         * This object holds an `Tk.Config` value for each config property keyed by name.
...
Tk.Configurator.prototype = {
    self: Tk.Configurator,
     ...
     * @param {Tk.Class} [mixinClass] The mixin class if the configs are from a mixin.
     * @private
     */
        ...
        for (name in config) {
            ...
            if (cfg) {
                ...
                if (!mixinClass && isCached && !cachedConfigs[name]) {
                    Tk.raise('Redefining config as cached: ' + name + ' in class: ' + Cls.$className);
                }
                //</debug>
            ...
            if (meta) {
                if (cfg.owner !== Cls) {
                    configs[name] = cfg = Tk.Object.chain(cfg);
                    cfg.owner = Cls;
                }
                Tk.apply(cfg, meta);
                delete cfg.$value;
    ...
    configure: function (instance, instanceConfig) {
        ...
        //
        // The merging in this loop cannot be completed by Tk.merge(), since we do NOT want to merge
            ...
            for (name in instanceConfig) {
                ...
                if (!cfg) {
                    ...
                    if (instance.$configStrict && (typeof field === 'function') && !field.$nullFn) {
                        ...
                        Tk.raise('Cannot override method ' + name + ' on ' + instance.$className + ' instance.');
                    }
                    //</debug>
            ...
            if (cfg) {
                if (cfg.merge) {
                    ...
                    if (baseValue && baseValue.constructor === Object) {
                        value = Tk.Object.merge(baseValue, value);
                    } else {
                        value = Tk.clone(value);
                    }
                }
    ...
    reconfigure: function (instance, instanceConfig, options) {
        ...
        for (name in instanceConfig) {
            ...
            if (this.deprecations[name]) {
                ...
                Tk.log.warn(this.deprecations[name]);
                if (!cfg) {
                    continue;
                }
            }
            ...
            if (cfg) {
                ...
                if (strict) {
                    if ((typeof prop === 'function') && !prop.$nullFn) {
                        //<debug>
                        Tk.Error.raise("Cannot override method " + name + " on " + instance.$className + " instance.");
                        //</debug>
                        continue;
                        //<debug>
                    } else {
                        if (name !== 'type') {
                            Tk.log.warn('No such config "' + name + '" for class ' +
                                instance.$className);
                        }
                        //</debug>
                    }
        ...
        for (i = 0, len = configList.length; i < len; i++) {
            ...
            if (cfg) {
                ...
                cfg = configPropMap[name] || Tk.Config.get(name);
                names = cfg.names;
    ...
    resolvePlatformConfig: function (instance, instanceConfig) {
        ...
        if (platformConfig) {
            keys = Tk.getPlatformConfigKeys(platformConfig);
            n = keys.length;

            if (n) {
                ret = Tk.merge({}, ret); // this deep copies sub-objects
