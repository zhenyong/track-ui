 ...
 *      });
 *
 * This uses the cached `Tk.Config` instance for the "foo" property.
 *
 ...
 *      });
 *
 * This creates a prototype chain to the cached "foo" instance of `Tk.Config` and applies
 * the `lazy` option to that new instance. This chained instance is then kept by the
 * `Tk.Configurator` for that class.
 * @private
 */
Tk.Config = function (name) {
// @define Tk.class.Config
// @define Tk.Config
...
Tk.Config.get = function (name) {
    var map = Tk.Config.map,
        ret = map[name] || (map[name] = new Tk.Config(name));
...
Tk.Config.prototype = {
     ...
     * When set as `true` the config property will be treated as a {@link Tk.Evented Evented Config}.
     * @private
     * @since 5.5.0
     */
     ...
     * will be applied.
     *
     * @cfg {Tk.Class} merge.mixinClass The mixin providing the `newValue` or `null` if
    ...
    mergeSets: function (newValue, oldValue, preserveExisting) {
        var ret = oldValue ? Tk.Object.chain(oldValue) : {},
            i, val;
